class DailysController < ApplicationController
  ...
  def home
    ...
    id = @user.id
    @dailys = Daily.where("user_id IN (?) OR user_id = ?", following_ids, id).eager_load( :mountain).preload({user: {profile: :avatar_attachment}}, :images_attachments).page(params[:page]).per(8)
  ...
  def own
    @dailys = @user.dailys.page(params[:page]).per(6).eager_load(:user, :mountain).preload(:images_attachments)
    respond_to do |format|
      format.html
      format.js
    end
